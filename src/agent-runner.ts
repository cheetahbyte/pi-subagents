...
export async function runAgent(
  ...
  const readmitToolNames = new Set([
    ...[...nestedToolNames, ...questionToolNames].filter(name => !disallowedSet?.has(name)),
    ...structuredToolNames,
  ]);
  ...
  if (noExtensions) {
    ...
    const denyTools = new Set<string>(
      EXCLUDED_TOOL_NAMES.filter((t) => !readmitToolNames.has(t)),
    );
  ...
  const sessionOpts: Parameters<typeof createAgentSession>[0] & {
    ...
    model,
    tools: sessionTools,
    customTools: [...nestedTools, ...questionTools, ...structuredTools],

...
export class AgentManager {
  ...
  resolveMention(name: string): MentionResolution | undefined {
    ...
    for (const record of this.agents.values()) {
      // The same definition every other user-facing surface uses: a workflow
      // child owns no handle and is not addressable by id even though it has
      // no parentAgentId — only its run may reach it.

...
describe("AgentManager — names as additive aliases", () => {
  ...
  it("refuses to resolve a workflow-owned child by id, while top-level ids still resolve", () => {
    // isTopLevelAgent defines every user-facing surface; the merge left the
    // byId fallback on parentAgentId alone, so a workflow child (which spawns
    // with no parentAgentId) was addressable through a boundary only its run
    // may cross — the same leak the nested-child test above pins by name.
    resolvedRun();
    manager = new AgentManager();
    const topId = manager.spawn(mockPi, mockCtx, "Explore", "go", {
      description: "go", isBackground: true,
    });
    const wfId = manager.spawn(mockPi, mockCtx, "Explore", "go", {
      description: "go", isBackground: true, workflowId: "wf-1",
    });

    expect(manager.getRecord(wfId)!.workflowId).toBe("wf-1");
    expect(manager.resolveMention(topId)).toMatchObject({ kind: "live", record: { id: topId } });
    expect(manager.resolveMention(wfId)).toBeUndefined();
  });

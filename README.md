...
- **Live widget UI** — persistent above-editor widget with animated spinners, live tool activity, token counts, and colored status icons. Configurable via `/agents → Settings → Widget`: `all` (every agent), `background` (default — hides foreground runs, which already render inline as the `Agent` tool result), or `off`. For [`pi-footer`](https://www.npmjs.com/package/pi-footer), add a **Pi Event Value** widget with ID `subagents` to show `󱚥 2` for running agents and `󱚠 1` for queued agents.
- **FleetView** — full-screen picker for `main`, workflow runs, and retained agents grouped by lifecycle status. Press `←` at an empty prompt, then `↑`/`↓` to select and `Enter` to view. Agents open in pi's native transcript when supported, with a conversation-overlay fallback; workflows open their inspector. Toggle via `/agents → Settings → Fleet view`
...
Press `←` at an empty prompt to open the full-screen picker:
...
On supported pi versions, selecting an agent displays its conversation in the native transcript. Type a message and press `Enter` to steer that agent; `Esc` at an empty prompt returns to the parent. A slash command or `@` mention also restores the parent view before dispatch. If native transcript replacement is unavailable, a live conversation overlay opens instead; closing that overlay returns to the picker. Disable FleetView via `/agents → Settings → Fleet view`.
...
- Only background children can ask. A foreground child's parent is blocked on the `Agent` call, so its question is refused. Workflow children also run through the blocking spawn path and cannot ask, even though the workflow itself runs in the background.
...
The match is on the exact tool names `Workflow` (Claude Code's), `workflow`, and `SubagentWorkflow` (ours), never a substring, so a `list_workflows` or `github_workflow_run` from some CI integration does not silently take the feature down. The check runs at `session_start` and nowhere earlier, because `getAllTools` throws during extension loading and load order means a check at registration time could not see an extension that has not loaded yet — so the tool is registered first and withdrawn from the active set through `setActiveTools`, which rebuilds the system prompt before any turn runs. When the other extension took the `SubagentWorkflow` name itself, pi's first-registration-wins rule already dropped ours, so there is nothing to withdraw and only the menu and the CLI flag come down.

**Tool description** (`toolDescriptionMode`, default `"compact"`): which Agent and SubagentWorkflow descriptions the LLM sees. `"compact"` keeps their executable contracts but removes recipes and repeated rationale. The Agent description is approximately 75% smaller; the workflow description drops from 19.4 KB to 4.2 KB (79%), reducing its estimated context cost from approximately 5,000 tokens to approximately 1,000 before the agent roster is inserted. `"full"` restores both rich Claude Code-style prompts. Parameter descriptions remain available in every mode. Changes apply on the next pi session.

`"custom"` registers your own Agent description from `<cwd>/.pi/agent-tool-description.md` (project) or `<agentDir>/agent-tool-description.md` (global; project wins), while SubagentWorkflow stays compact. The file is read once at tool registration, so edits also apply on the next pi session. Dynamic parts stay live via placeholders — a static agent list would go stale the moment you add a custom agent:
...
Placeholders: `{{typeList}}` (full per-agent descriptions), `{{compactTypeList}}` (first sentence each), `{{agentDir}}`, `{{isolationGuideline}}`, `{{scheduleGuideline}}` and `{{handoffGuideline}}` (each expands with its own leading newline + `- ` bullet when the matching feature is on — place them directly after your last rule line; empty when [worktree isolation](#turning-worktrees-off) / scheduling is off, or when no `handoff` skill is installed in a skills root). Unknown placeholders are left verbatim with a stderr warning; a missing or empty file falls back to `"full"` with a warning. Note the usual trust umbrella: a project-level file shapes the orchestrator's prompt, same as project agents and extensions do.
...
src/
  ...
  ui/
    ...
    fleet-list.ts         # FleetView: full-screen workflow and grouped-agent picker
    transcript-override.ts # Guarded native transcript replacement; overlay fallback
